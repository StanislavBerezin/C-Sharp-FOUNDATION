...
namespace ConsoleUI.WithGenerics
{
    ...
    {
        //return type is a list(type not clear before this method is called)
        //where T : => it creates limitations on what can call this method,
        //and in this case it has to be a class not an int or string, new() is for empty constructor
        //for the class we are making
        public static List<T> LoadFromTextFile<T>(string filePath) where T : class, new()
        {
            //creating this type of a list
            var lines = System.IO.File.ReadAllLines(filePath).ToList();
            List<T> output = new List<T>();
            //here we know that this class has an empty constructor, hence no need to pass the arguments
            //without limitations we set above we wouldnt know. 
            T entry = new T();

            ///here we get the type of that List and we get properties from that list,
            /// like logEntry has its own props and Person too. SO this var gets FirstName, LastName, and isAlive
